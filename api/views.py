...
def match_user_algorithm(user, settings):
    ...
    if settings.get('radius', None):
        if user.profile.lat and user.profile.lng:
            ...
            users = users.filter(lat__gte=x1, lat__lte=x2,
                                 lng__gte=y1, lng__lte=y2)
    return users
...
class GetMatchingUsersView(APIView):
    ...
    def get(self, request):
        ...
        users = match_user_algorithm(
            request.user, request.data.get("args", {}))
        if len(users) > 0:
            serialized_users = []
            for user in users:
                if user.visible:
    ...
    def get(self, request):
        ...
        if len(received_swipes) > 0:
            ...
            for swipe in received_swipes:
                sent_swipes = Swipe.objects.filter(
                    swiped=swipe.swiper, swiper=profile)
                if len(sent_swipes) > 0:
                    swipe = sent_swipes[0]
                    if swipe.swiper.visible:
                        data = PrivateProfileSerializer(swipe.swiped).data
                        matched_swipers.append(data)
                    else:
                        data = PublicProfileSerializer(swipe.swiped).data
                        matched_swipers.append(data)
    ...
    def post(self, request):
        ...
        if len(users) > 0:
            ...
            swipe1 = Swipe.objects.filter(
                swiper=profile, swiped=user, swipe_type="right")
            swipe2 = Swipe.objects.filter(
                swiper=user, swiped=profile, swipe_type="right")
            if len(swipe1) > 0 and len(swipe2) > 0:
                Message.objects.create(
                    sender=profile, receiver=user, message=request.data['message'])
            else:
...
def validate_profile_changes(changes):
    valid_keys = ['work', 'job_title', 'school',
                  'education_level', 'hometown', 'bio', 'visible']
    errors = []
